...
const Maps = ({ onStationsUpdate, onMarkerMapUpdate }) => {
                    ...
                    if (detailBtn) {
                      detailBtn.addEventListener("click", async () => {
                        try {
                          ...
                          console.log(
                            "Fetched station details:",
                            stationDetails
                          );

                          // 평균 평점 API 호출
                          const averageRatings =
                            await fetchAverageRatings(stationCode);
  ...
  const fetchAverageRatings = async (stationCode) => {
    try {
      const response = await fetch(
        `/api/gas-stations/${stationCode}/average-ratings`
      );
      if (!response.ok) {
      ...
      {isDetailPopupVisible && selectedStation && (
        <div className="detail-popup">
          ...
          <table className="price-table">
            <thead>
              <tr>
                <th style={{ fontWeight: "bold", color: "rgb(214, 0, 0)" }}>
                  가격
                </th>
                <th>업데이트 날짜</th>
              </tr>
            ...
            <tbody>
              <tr>
                <td style={{ fontWeight: "bold", color: "rgb(214, 0, 0)" }}>
                  {selectedStation.oilPrice}원
                </td>
          ...
          <p>
            ...
            {renderStars(
              Math.round(
                parseFloat(selectedStation.averageRatings?.restroomAvg || "0.0")
              )
            )}{" "}
            <span style={{ color: "blue", fontWeight: "bold" }}>
              평균 {selectedStation.averageRatings?.restroomAvg || "0.0"}점
            </span>
          </p>
            ...
            <tr>
              ...
              <td>
                {renderStars(
                  Math.round(
                    parseFloat(
                      selectedStation.averageRatings?.restroomAvg || "0.0"
                    )
                  )
                )}
              </td>
            ...
            <tr>
              ...
              <td>
                {renderStars(
                  Math.round(
                    parseFloat(
                      selectedStation.averageRatings?.accessAvg || "0.0"
                    )
                  )
                )}
              </td>
            ...
            <tr>
              ...
              <td>
                {renderStars(
                  Math.round(
                    parseFloat(
                      selectedStation.averageRatings?.priceAvg || "0.0"
                    )
                  )
                )}
              </td>
          ...
          <div className="additional-info">
            {selectedStation.cvsYn === "Y" && (
              <div className="additional">
                <img src={store} className="icon" /> 편의점
              </div>
            )}
            {selectedStation.carWashYn === "Y" && (
              <div className="additional">
                <img src={good} className="icon" /> 세차장
              </div>
            )}
            {selectedStation.mainYn === "Y" && (
              <div className="additional">
                <img src={repair} className="icon" /> 정비소
              </div>
            )}
          </div>
          <br />
          <div className="action-buttons">
            ...
            <button
              style={{ backgroundColor: "rgb(32,32,88)", color: "white" }}
